...
void serialJalan();//GtkWidget *wid, gpointer ptr);
 ...
 gboolean timerTick(__attribute__((unused)) gpointer userData)
 {
     ...
     serialJalan();//timer,sambung);		//seharusnya bikin fungsi sendiri yang gak perlu pakai gwidget
...
void sambungserial(GtkWidget *wid, gpointer ptr)
{
...
int main( int argc, char **argv )
{
  ...
  sambung = gtk_button_new_with_label ("sambung"); 
  g_signal_connect (sambung, "clicked", G_CALLBACK (sambungserial), NULL); 
  ...
  gtk_box_pack_start (GTK_BOX (box), lbl, TRUE, TRUE, 0); 	//label
  gtk_box_pack_start (GTK_BOX (box), comb, TRUE, TRUE, 0); 	//combo /dev/tty
  gtk_box_pack_start (GTK_BOX (box), combBaud, TRUE, TRUE, 0); 	//combo baudrat
  ...
  gtk_container_add(GTK_CONTAINER (box), sambung);
